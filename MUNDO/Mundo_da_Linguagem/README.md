...
**Lote 1 — Numerais** (14º SubMundo, 8º item da seção 5): 50 perguntas (`temas/numerais_lote1.json`): tipos (cardinal, ordinal, multiplicativo, fracionário), ordinais (`vigésimo` a `centésimo`, `quingentésimo`), multiplicativos (`triplo` a `cêntuplo`), fracionários (`um terço`, `um onze avos`, `três quartos`), coletivos numéricos (`dúzia`, `biênio`, `triênio`, `década`, `século`), escrita por extenso (`e` entre grupos: `mil e cem`, `mil cento e um`), `ambos`, leitura de algarismos romanos (`Pedro II`, `Luís XIV`, `século XXI`) e concordância (`trezentas casas`). Evitei formas com variante aceita (`dois mil vinte e cinco`, `lustro` x `quinquênio`). Migration `100`, carga `content/linguagem_tema_numerais.json`.

**Lote 1 — Interjeições** (15º SubMundo, também do 8º item): 50 perguntas (`temas/interjeicoes_lote1.json`): o que cada interjeição exprime (dor, alegria, alívio, admiração, desejo, aplauso, afugentamento, advertência, animação, saudação, apelo, repulsa, impaciência, despedida, desculpa), locuções interjetivas (`Meu Deus!`, `Ora bolas!`, `Graças a Deus!`), palavras de outras classes usadas como interjeição (`Silêncio!`), pontuação, função emotiva, `oxalá`/`tomara` com subjuntivo e pares parecidos (`hei`/`ei`, `ah`/`há`, `ó`/`oh`, `hein`). Evitei interjeições de sentido ambíguo (`psiu`, `viva`, `ah`). Migration `100`, carga `content/linguagem_tema_interjeicoes.json`.

**Lote 1 — Sintaxe** (16º SubMundo, 9º item da seção 5): 50 perguntas (`temas/sintaxe_lote1.json`): sujeito (simples, composto, oculto, indeterminado, oração sem sujeito), predicado (verbal, nominal, verbo-nominal), transitividade, objetos, adjuntos, complemento nominal, aposto, vocativo, predicativos, agente da passiva, voz ativa/passiva e `se` apassivador/indeterminador. Evitei casos discutíveis na análise sintática. Migration `101`, carga `content/linguagem_tema_sintaxe.json`.

**Lote 1 — Orações Coordenadas e Subordinadas** (17º SubMundo, também do 9º item): 50 perguntas (`temas/oracoes_coordenadas_e_subordinadas_lote1.json`): coordenadas (assindéticas e sindéticas aditivas, adversativas, alternativas, conclusivas, explicativas), subordinadas substantivas (subjetiva, objetivas, predicativa, completiva nominal, apositiva), adjetivas (restritiva x explicativa), adverbiais (causal, condicional, concessiva, temporal, final, consecutiva, conformativa, modal, proporcional, comparativa), orações reduzidas e `que` conjunção integrante x pronome relativo. Evitei classificações em que as gramáticas divergem. Migration `101`, carga `content/linguagem_tema_oracoes_coordenadas_e_subordinadas.json`.

**Lote 1 — Semântica** (18º SubMundo, 10º e último item da seção 5): 50 perguntas (`temas/semantica_lote1.json`): sinônimos e antônimos, homonímia (homônimas, homófonas, homógrafas), paronímia (`emigrar/imigrar`, `infligir/infringir`, `eminente/iminente`), polissemia, denotação e conotação, hiperônimo e hipônimo, campo semântico, palavras cognatas, ambiguidade, arcaísmo, neologismo e estrangeirismo. Evitei pares em que os dicionários divergem entre polissemia e homonímia (`banco`, `cabo`). Migration `102`, carga `content/linguagem_tema_semantica.json`.

**Lote 1 — Figuras de Linguagem** (19º SubMundo, também do 10º item): 50 perguntas (`temas/figuras_de_linguagem_lote1.json`): metáfora, comparação, metonímia, catacrese, sinestesia, antítese, oxímoro, hipérbole, eufemismo, ironia, prosopopeia, gradação, pleonasmo, elipse, assíndeto, polissíndeto, aliteração, onomatopeia, anáfora, apóstrofe, antonomásia e hipérbato. Evitei classificações que variam entre autores (`parte pelo todo` como metonímia ou sinédoque, `paradoxo` x `oxímoro`, silepse, anacoluto). Migration `102`, carga `content/linguagem_tema_figuras_de_linguagem.json`. **Com este lote, todos os temas da seção 5 do documento têm lote 1 (pendente só a revisão de Rhoney e a carga).**
